...
export default function MembershipSet(props) {
  ...
  const changeSetHandler = (event) => {
    setSet(event.target.value);

    props.updateSetMembership(event.target.value);
  };
    ...
    return (
      <Box>
        <FormControl variant="outlined" className={classes.accountType}>
          ...
          <FormHelperText>Choose A Set(Year of Graduation)</FormHelperText>
        </FormControl>
      </Box>
    );
  };

  const renderCategoryField33 = ({
    input,
    label,
