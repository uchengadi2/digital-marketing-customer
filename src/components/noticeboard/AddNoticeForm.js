...
import { CREATE_NOTICE } from "../../actions/types";
...
const AddNoticeForm = (props) => {
  ...
  const [noticeRefNo, setNoticeRefNo] = useState(
    "NT-" + Math.floor(Math.random() * 1000000) + "-" + "OS"
  );
  ...
  const onSubmit = (formValues) => {
    ...
    form.append("noticeRefNo", noticeRefNo);
    ...
    if (formValues) {
      const createForm = async () => {
        ...
        const response = await api.post(`/noticeboards`, form);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `You have successfully posted a new notice!!!`
          );
          props.updateNoticeInfoHandler();
          props.handleNewNoticeDialogForm();
          setLoading(false);
        } else {
    ...
    <Box className={classes.root}>
      <Grid item container justifyContent="center">
        <FormLabel
          ...
          component="legend"
        >
          <Typography variant="h5">New Notice Form</Typography>
        </FormLabel>
      </Grid>
      <Box
        component="div"
        ...
        >
          <Grid item>
            <Field
              ...
              component={renderNoticeTypeField}
          ...
          <Grid item>
            <Field
              label="Notice Video Link(YouTube Video Link)"
              id="video"
              name="video"
              type="text"
              //defaultValue={props.user.name}
          ...
          <Grid item>
            <Field
              ...
              type="text"
              //defaultValue={props.user.name}
              component={renderTargetAudienceField}
              style={{ marginTop: 3, width: 500 }}
            />
          </Grid>
