...
import background from "./../../assets/images/headers/event2.png";
...
const Events = (props) => {
  ...
  const [updateEvent, setUpdateEvent] = useState("all");

  const [updateEventInfo, setUpdateEventInfo] = useState(false);

  const updateEventHandler = (value) => {
  ...
  const updateEventInfoHandler = () => {
    setUpdateEventInfo((prevState) => !prevState);
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (updateEvent === "all") {
        ...
        const response = await data.get("/events?sort=asc");
        const workingData = response.data.data.data;
        workingData.map((event) => {
          allData.push({
            id: event._id,
            title: event.title,
        ...
        setEventsList(allData);
      } else {
        //data.defaults.headers.common["Authorization"] = `Bearer ${token}`;
        const response = await data.get("/events?sort=asc", {
          params: { type: updateEvent },
        });
        const workingData = response.data.data.data;
        workingData.map((event) => {
          allData.push({
    ...
    <>
      ...
      <Grid container direction="row" className={classes.root}>
        ...
        <EventHeader
          token={props.token}
          userId={props.userId}
          updateEventInfoHandler={updateEventInfoHandler}
        />
        <EventTypes
          token={props.token}
          userId={props.userId}
